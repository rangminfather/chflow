...
/**
 * 여러 스프레드시트 프로그램이 만든 비표준 OOXML을 ExcelJS가 다시 읽을 수 있는
 * 표준 .xlsx로 재작성한다. 원본이 xlsx가 아니거나 복구할 수 없으면 null을 반환한다.
 */
export async function normalizeXlsxForStorage(
  bytes: Buffer | ArrayBuffer | Uint8Array,
): Promise<Buffer | null> {
  if (detectWorkbookFormat(bytes) !== "xlsx") return null;
  const workbook = await loadWorkbook(bytes);
