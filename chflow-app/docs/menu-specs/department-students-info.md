...
|---:|---|---|---|---|
| 1 | 부서홈 메뉴 노출 | `app/departments/d/[id]/page.tsx` | MENU_CATEGORIES의 학생정보관리 항목으로 정의된다. | 사용자 부서 등급과 부서 조건이 맞으면 카드가 표시된다. |
| 2 | 화면 이동 | `app/departments/d/[id]/page.tsx` | implemented=false라 클릭 시 준비중 토스트를 띄운다. | 학생 명부와 인적사항 관리 예정 메뉴다. |
