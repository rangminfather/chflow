...
| 2 | 출결 체크 그리드 | `app/departments/d/[id]/attendance/page.tsx` | 반별 그룹 × 해당 월 주일 그리드. 출결 칸 탭 시 출석→결석→출석인정 순환 (`edu_set_student_attendance`). | 전 반 학생을 한 화면에서 체크·수정한다. 월합계는 출석/결석/출석인정 카운트. |
| 3 | 달란트 체크 현황 | `app/departments/d/[id]/attendance/page.tsx` | `get_dept_weekly_extra` + `list_talent_rules`로 주별 체크 개수·합계 점수를 읽기 전용 표시한다. | 체크 자체는 달란트통장 메뉴에서 수행. 배지에 마우스오버 시 항목 상세. |
