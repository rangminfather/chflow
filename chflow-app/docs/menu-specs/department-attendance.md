...
| 메뉴명 | 출결 통합 조회 |
| 대분류 | 부서 내부 메뉴 > 행정관리 |
| 간단 설명 | 전 반 학생 출결을 조회하고 관리자가 강제 수정한다. |
...
| 2 | 화면 이동 | `app/departments/d/[id]/attendance/page.tsx` | 클릭 시 /departments/d/[id]/attendance로 이동한다. | 전 반 학생 출결을 조회하고 관리자가 강제 수정한다. |
