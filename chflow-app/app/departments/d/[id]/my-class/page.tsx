...
export default function MyClassPage() {
  ...
  const [familyCandidates, setFamilyCandidates] = useState<FamilyMatchCandidate[]>([]);
  const [selectedFamilyMemberId, setSelectedFamilyMemberId] = useState<string | null>(null);
  const [selectedRelativeIds, setSelectedRelativeIds] = useState<string[]>([]);
  const [familyReviewOpen, setFamilyReviewOpen] = useState(false);
  const [skipNewFriendFamilyCheck, setSkipNewFriendFamilyCheck] = useState(false);
  ...
  async function loadStudents(teacherId: string | null, classNos?: string[]) {
    ...
    const { data: studentRows, error: studentErr } = await supabase
      .from("edu_students")
      .select("id, department_id, student_no, name, student_type, mgmt_status, grade, grade_year, class_no, school_name, is_active, order_no, member_id, teacher_id, phone, birth_date, gender, address, parent_contacts, notes, photo_url")
  ...
  async function openNewFriend() {
    ...
    setSkipNewFriendFamilyCheck(false);
  ...
  async function saveNewFriend() {
    ...
    if (!newFriend.member_id && newFriend.family.length === 0 && !skipNewFriendFamilyCheck) {
      const opened = await openNewFriendFamilyReview();
      if (opened) return;
    }
    ...
    if (newFriend.member_id && newFriendId) {
      try {
        const { data: savedFriend } = await supabase
          .from("edu_new_friends")
          .select("student_id")
          ...
          .maybeSingle();
        if (savedFriend?.student_id) {
          await linkStudentFamily(savedFriend.student_id, newFriend.member_id, family);
        }
      } catch (familyError) {
        showToast(`새친구는 등록됐지만 요람 가족 연결 실패: ${familyError instanceof Error ? familyError.message : "오류"}`);
      }
    }
  ...
  async function openNewFriendFamilyReview() {
    if (!newFriend.name.trim()) { showToast("이름을 먼저 입력하세요"); return false; }
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) { router.replace("/login"); return false; }
    const birthDate = (() => {
      const by = Number(newFriend.birthYear);
      if (!newFriend.birthYear || !Number.isFinite(by) || by < 2000 || by > 2100) return null;
      const bm = Math.min(12, Math.max(1, Number(newFriend.birthMonth) || 1));
      const bd = Math.min(31, Math.max(1, Number(newFriend.birthDay) || 1));
      return `${by}-${String(bm).padStart(2, "0")}-${String(bd).padStart(2, "0")}`;
    })();
    const response = await fetch("/api/edu/family-match", {
      ...
      body: JSON.stringify({
        dept_id: deptId,
        student: { name: newFriend.name.trim(), birth_date: birthDate, phone: newFriend.mobile || null, gender: newFriend.gender === "남" ? "M" : newFriend.gender === "여" ? "F" : null },
      }),
    });
    const result = await response.json();
    const candidates = (result.results?.[0]?.candidates || []) as FamilyMatchCandidate[];
    if (!response.ok || !result.ok) { showToast(result.error || "요람 가족 후보를 찾지 못했습니다"); return false; }
    if (candidates.length === 0) { showToast("매칭되는 요람 가족 후보가 없습니다"); return false; }
    setFamilyCandidates(candidates);
    setSelectedFamilyMemberId(candidates[0].member_id);
    setSelectedRelativeIds(candidates[0].family.filter((family) => family.default_selected).map((family) => family.relative_id));
    setFamilyReviewOpen(true);
    return true;
  }

  function applyNewFriendFamilyReview() {
    const candidate = familyCandidates.find((item) => item.member_id === selectedFamilyMemberId);
    if (!candidate) return;
    const family = candidate.family
      .filter((entry) => selectedRelativeIds.includes(entry.relative_id))
      .map((entry) => ({ relative_id: entry.relative_id, name: entry.name, relation: entry.relation, phone: entry.phone || "" }));
    setNewFriend((current) => ({
      ...current,
      member_id: candidate.member_id,
      mobile: current.mobile || formatPhone(candidate.phone || ""),
      address: current.address || candidate.address || "",
      family: mergeFamilyEntries(current.family, family),
    }));
    setSkipNewFriendFamilyCheck(true);
    setFamilyReviewOpen(false);
  }

  async function linkStudentFamily(studentId: string, memberId: string, family: FamilyEntry[]) {
    const { data: { session } } = await supabase.auth.getSession();
    if (!session) return;
    const response = await fetch("/api/edu/link-student-family", {
      method: "POST",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${session.access_token}` },
      body: JSON.stringify({ dept_id: deptId, student_id: studentId, member_id: memberId, family }),
    });
    const result = await response.json();
    if (!response.ok || !result.ok) throw new Error(result.error || "요람 가족 연결 실패");
  }
    ...
    <div style={pageStyle}>
              ...
              <div className="grid gap-4 p-4 lg:grid-cols-2">
                ...
                <div className="rounded-lg border border-hairline bg-surface p-4">
                  ...
                  <InfoField label="부모님 연락처" editMode={editMode} value={parentContactsLabel(draft.parent_contacts)}>
                    <ParentContactsEditor contacts={draft.parent_contacts} onChange={(parent_contacts) => updateDraft("parent_contacts", parent_contacts)} />
                  </InfoField>
                  ...
                  <InfoField label="비고" editMode={editMode} value={draft.notes || "미등록"}>
                    <textarea value={draft.notes} onChange={(event) => updateDraft("notes", event.target.value)} maxLength={1000} rows={3} placeholder="특이사항을 기록하세요" className={`${inputClass} resize-y`} />
                  </InfoField>
                </div>
      ...
      {showNewFriend && (
        <NewFriendModal
          ...
          onFindFamily={openNewFriendFamilyReview}
        />
      )}
      ...
      {familyReviewOpen && (
        <FamilyReviewModal
          ...
          onClose={() => setFamilyReviewOpen(false)}
          onSkip={() => { setSkipNewFriendFamilyCheck(true); setFamilyReviewOpen(false); }}
          onApply={applyNewFriendFamilyReview}
          onSelectCandidate={(memberId) => {
            const candidate = familyCandidates.find((item) => item.member_id === memberId);
            setSelectedFamilyMemberId(memberId);
            setSelectedRelativeIds(candidate?.family.filter((family) => family.default_selected).map((family) => family.relative_id) || []);
          }}
          onToggleRelative={(relativeId) => setSelectedRelativeIds((current) => current.includes(relativeId) ? current.filter((id) => id !== relativeId) : [...current, relativeId])}
          onToggleAll={(checked) => {
            const candidate = familyCandidates.find((item) => item.member_id === selectedFamilyMemberId);
            setSelectedRelativeIds(checked ? (candidate?.family.map((family) => family.relative_id) || []) : []);
          }}
        />
      )}
          ...
          <div className="mb-2 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="inline-flex min-h-8 items-center rounded-md border border-amber-300 bg-amber-50 px-2.5 text-[12.5px] font-extrabold text-amber-900"
              >
                요람 가족 찾기
              </button>
              <button
    ...
    <div className="space-y-2">
      {contacts.map((contact, index) => (
        <div key={index} className="grid grid-cols-[76px_1fr_1fr_36px] gap-1.5 max-sm:grid-cols-1">
          ...
          <button type="button" aria-label="부모님 연락처 삭제" onClick={() => onChange(contacts.filter((_, i) => i !== index))} className="inline-flex h-11 w-9 items-center justify-center rounded-md border border-hairline bg-card text-ink-faint max-sm:w-full">
            <X size={14} strokeWidth={2.2} />
          </button>
        </div>
      ))}
...
function normalizeParentContacts(value: unknown): ParentContact[] {
  ...
  return value
    .map((item) => {
      ...
      return {
        relation: typeof row.relation === "string" && row.relation ? row.relation : "보호자",
...
function parentContactsLabel(contacts: ParentContact[]) {
  ...
  return contacts
    .map((contact) => [contact.relation, contact.name, contact.phone ? formatPhone(contact.phone) : ""].filter(Boolean).join(" "))
    .join(" · ");
}

function normalizeStudentType(value: string | null | undefined) {
  return ["정", "체험"].includes(value || "") ? (value as string) : "정";
}
...
function FamilyReviewModal({
  ...
  return (
    <div className="fixed inset-0 z-[1200] flex items-center justify-center bg-ink/50 p-4" onMouseDown={onClose}>
      <div className="max-h-[90vh] w-full max-w-[860px] overflow-y-auto rounded-lg bg-card p-5" onMouseDown={(event) => event.stopPropagation()}>
        <div className="mb-4 flex items-start justify-between gap-3">
          ...
          <button type="button" onClick={onClose} className="inline-flex h-9 w-9 items-center justify-center rounded-md border border-hairline bg-card text-ink-soft"><X size={16} /></button>
        </div>
        <div className="grid gap-3 md:grid-cols-[280px_1fr]">
          <div>
            <button type="button" onClick={onSkip} className="mb-2 w-full rounded-md border border-hairline bg-card px-3 py-2 text-left text-[13px] font-extrabold text-ink">해당없음 <span className="text-ink-faint">· 수동 입력</span></button>
