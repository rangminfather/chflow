...
import { Lock, Medal, Printer, TrendingUp, X } from "lucide-react";
...
export default function TalentStatsPage() {
  ...
  const printItems = useMemo(() => {
    ...
    const filtered = classFilter ? source.filter((item) => item.classLabel === classFilter) : source;
...
// ─────────────────────────────────────────
// 잔치용 통장 일괄 출력 — A4 세로 한 장에 8매(2×4), 점선 재단선.
// 카드 = 이름·반·총 달란트 + 시장에서 볼펜으로 차감하는 빈 칸.
// 종이 표면이므로 배경은 var(--paper) (다크모드에서도 흰 종이).
// ─────────────────────────────────────────
function PassbookPrintOverlay({ periodLabel, items, onClose }: {
  periodLabel: string;
  items: StudentTotal[];
  onClose: () => void;
}) {
        ...
        {items.map((item) => (
          <div key={item.id} className="tp-card">
            ...
            <div className="tp-card-total">
              ...
              <span className="tp-card-total-value">{fmt(item.total)}</span>
