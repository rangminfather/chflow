...
function AdminMembersPage() {
  ...
  const plainOptions = useMemo(() => {
    const arr: { name: string; order: number }[] = [];
    const seen = new Set<string>();
    dirTree.forEach(r => {
      if (r.plain_name && !seen.has(r.plain_name)) {
        seen.add(r.plain_name);
        arr.push({ name: r.plain_name, order: r.plain_order ?? 99 });
      }
    });
  ...
  const handleSave = async (move?: MoveTarget) => {
    ...
    if (move?.add_parent_id) {
      const { error: relErr } = await supabase.rpc("add_member_relation", {
        p_subject_id: editing.id,
        p_relative_id: move.add_parent_id,
        p_kind: "parent",
        p_role: move.add_parent_role || null,
      });
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Noto Sans KR', sans-serif", padding: 16 }}>
      ...
      {/* Edit Modal */}
      {editing && <EditModal member={editing} setMember={setEditing} dirTree={dirTree} plainOptions={plainOptions} plainLabel={plainLabel} onSave={handleSave} onClose={() => setEditing(null)} />}
...
function EditModal({ member, setMember, dirTree, plainOptions, plainLabel, onSave, onClose }: {
  ...
  dirTree: DirRow[];
  plainOptions: { name: string; order: number }[];
  plainLabel: (name: string) => string;
  ...
  const searchParent = async () => {
    ...
    setParentCandidates((data || []).filter((c: any) => !c.is_child && c.id !== member.id));
  ...
  const handleSave = () => {
    ...
    if (member.is_child && parent) {
      parentInfo.add_parent_id = parent.id;
      parentInfo.add_parent_role = parent.gender === "M" ? "father" : parent.gender === "F" ? "mother" : undefined;
    }

    // 평원 안 정함 → 어디에도 소속 안 둠 (본인만)
    if (!movePlain) {
      onSave({ clear_household: true, ...parentInfo });
      return;
    }
    ...
    const move: MoveTarget = { ...parentInfo };
          ...
          <label style={{ display: "flex", alignItems: "flex-end", gap: 6, fontSize: 12, fontWeight: 600, color: "#475569", paddingBottom: 10 }}>
            <input type="checkbox" checked={member.is_child}
              onChange={(e) => {
                ...
                setMember({ ...member, is_child: next });
              }} />
            자녀
          </label>
        </div>
...
// ============ Create Modal ============
function CreateModal({ dirTree, plainOptions, plainLabel, onClose, onCreated }: {
  dirTree: DirRow[];
  plainOptions: { name: string; order: number }[];
  plainLabel: (name: string) => string;
  onClose: () => void;
