...
function relationLabel(kind: string, role: string | null) {
  ...
  if (role === "grandfather" || role === "paternal_grandfather" || role === "maternal_grandfather") return "조부";
  if (role === "grandmother" || role === "paternal_grandmother" || role === "maternal_grandmother") return "조모";
...
function defaultSelected(kind: string) {
  return kind === "parent";
}
...
async function requireDeptAdmin(req: NextRequest, deptId: string) {
  ...
  const { data: gradeData, error: gradeErr } = await client.rpc("get_user_grade", { p_dept_id: deptId });
...
async function fetchMembersByIds(admin: ReturnType<typeof adminClient>, ids: string[]) {
  ...
  const { data, error } = await admin
    .from("members")
    .select("id,name,phone,birth_date,gender,photo_url,address,household_id")
    .in("id", unique);
...
async function familyFor(admin: ReturnType<typeof adminClient>, memberId: string) {
  ...
  const rows = ((data || []) as Relation[]).filter((row) =>
    ["parent", "grandparent", "great_grandparent", "spouse", "sibling"].includes(row.kind),
  );
  const relativeIds = rows.map((row) => (row.subject_id === memberId ? row.relative_id : row.subject_id));
  const members = await fetchMembersByIds(admin, relativeIds);
  ...
  return rows
    .map((row) => {
      ...
      return {
        ...
        kind: row.kind,
        role: row.role,
        default_selected: defaultSelected(row.kind),
      };
    })
    .filter(Boolean);
}
...
async function childIdsFromParent(admin: ReturnType<typeof adminClient>, input: MatchInput) {
  ...
  const parentIds = ((parents || []) as Pick<Member, "id" | "phone">[])
    .filter((parent) => !input.parent_phone || phoneMatch(parent.phone, input.parent_phone) > 0)
    .map((parent) => parent.id);
  if (parentIds.length === 0) return [];

  const { data: rels, error: relError } = await admin
    .from("member_relations")
    .select("subject_id,relative_id,kind")
    .in("relative_id", parentIds)
    .in("kind", ["parent", "grandparent", "great_grandparent"]);
...
async function matchOne(admin: ReturnType<typeof adminClient>, deptId: string, input: MatchInput) {
  ...
  const memberIds = Array.from(candidateMap.keys());
  ...
  if (memberIds.length > 0) {
    ...
    ((data || []) as { member_id: string | null }[]).forEach((row) => {
      if (row.member_id) linked.add(row.member_id);
    });
  }
  ...
  const candidates = await Promise.all(Array.from(candidateMap.values()).map(async (member) => {
    ...
    const family = await familyFor(admin, member.id);
    return {
      member_id: member.id,
      name: member.name || "",
      phone: member.phone || "",
      ...
      already_in_department: linked.has(member.id),
      score,
...
export async function POST(req: NextRequest) {
  ...
  if (inputs.length === 0) return NextResponse.json({ ok: true, results: [] });

  try {
    const admin = adminClient();
    const results = [];
    for (const input of inputs) {
      results.push(await matchOne(admin, body.dept_id, input));
    }
