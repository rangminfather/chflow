...
function StatCard({ stat }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: "12px 14px", minWidth: 0, overflow: "hidden" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 6 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          <div style={{ fontSize: 11, color: "#94a3b8", fontWeight: 500, marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{stat.label}</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: "#1e293b", letterSpacing: -0.5, whiteSpace: "nowrap" }}>{stat.value}</div>
          <div style={{ fontSize: 10, color: "#64748b", marginTop: 2, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{stat.sub}</div>
        </div>
...
function WidgetCard({ widget }) {
  return (
    <div style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 10, padding: 14, minWidth: 0, overflow: "hidden" }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: "#334155", marginBottom: 10, display: "flex", alignItems: "center", gap: 6, whiteSpace: "nowrap" }}>
      ...
      {widget.items && (
        <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          {widget.items.map((item, i) => (
            <div key={i} style={{ fontSize: 11, color: "#475569", padding: "6px 8px", background: "#f8fafc", borderRadius: 6, border: "1px solid #f1f5f9", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{item}</div>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div className="stats-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 8 }}>
        {dash.stats.map((s, i) => <StatCard key={i} stat={s} />)}
      </div>
      <div>
        <div style={{ fontSize: 11, fontWeight: 600, color: "#94a3b8", marginBottom: 8, letterSpacing: 0.5 }}>빠른 메뉴</div>
        <div className="quick-menu-grid" style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 6 }}>
          {dash.quickMenu.map((m, i) => (
            <div key={i} style={{
              padding: "10px 6px", background: "#fff", border: `1.5px solid ${roleColor}22`, borderRadius: 8,
              fontSize: 11, fontWeight: 600, color: roleColor, textAlign: "center", cursor: "pointer",
              whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
            }}>{m}</div>
          ))}
        </div>
      </div>
      <div className="widgets-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 10 }}>
        {dash.widgets.map((w, i) => <WidgetCard key={i} widget={w} />)}
      </div>
    </div>
  );
}
...
function SitemapView({ selectedRole }) {
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div className="sitemap-grid" style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 16 }}>
        {sitemapData.map((group, gi) => (
          <div key={gi} style={{ background: "#fff", border: "1px solid #e2e8f0", borderRadius: 12, overflow: "hidden" }}>
            <div style={{ padding: "10px 14px", background: group.color, display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontSize: 13, fontWeight: 700, color: "#fff" }}>{group.group}</span>
              <span style={{ fontSize: 10, color: "rgba(255,255,255,0.7)" }}>{group.pages.length}개</span>
            </div>
            <div style={{ padding: 8, display: "flex", flexDirection: "column", gap: 5 }}>
              {group.pages.map((page, pi) => {
                const accessible = selectedRole ? roleAccess[selectedRole]?.includes(page.name) : true;
                return (
                  <div key={pi} style={{ display: "flex", alignItems: "center", gap: 6, padding: "7px 8px", borderRadius: 6, background: accessible ? "#f0fdf4" : "#fef2f2", border: `1px solid ${accessible ? "#bbf7d0" : "#fecaca"}`, opacity: accessible ? 1 : 0.5 }}>
                    <span style={{ fontSize: 11, flexShrink: 0 }}>{accessible ? "✅" : "🔒"}</span>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: 11.5, fontWeight: 600, color: "#1e293b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{page.name}</div>
                      {page.desc && <div style={{ fontSize: 9.5, color: "#64748b", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{page.desc}</div>}
                    </div>
                    <div style={{ display: "flex", gap: 2, flexShrink: 0 }}>
                      {ROLES.filter(r => roleAccess[r.id]?.includes(page.name)).map(r => (
                        <div key={r.id} style={{ width: 5, height: 5, borderRadius: "50%", background: r.color, opacity: selectedRole && selectedRole !== r.id ? 0.2 : 1 }} />
                      ))}
                    </div>
                  </div>
                );
...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // 권한에 맞게 초기 역할 세팅
      if (profile.role && ROLES.some(r => r.id === profile.role)) {
    ...
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "'Noto Sans KR', -apple-system, sans-serif", color: "#1e293b" }}>
      ...
      <style>{`
        @media (max-width: 768px) {
          ...
          .header-tabs .tab-label-full { display: none; }
          .header-tabs .tab-label-short { display: inline; }
          .hamburger-btn { display: flex !important; }
          .main-content { padding: 12px !important; }
          .tab-btn { padding: 6px 10px !important; font-size: 11px !important; }
          .user-info .user-name, .user-info .user-sub { display: none !important; }
          .user-info { padding-left: 4px !important; gap: 4px !important; }
          ...
          .quick-menu-grid { grid-template-columns: repeat(2, 1fr) !important; gap: 6px !important; }
          .widgets-grid { grid-template-columns: 1fr !important; }
          .sitemap-grid { grid-template-columns: 1fr !important; gap: 10px !important; }
          .browser-chrome-url { font-size: 9px !important; }
          .sitemap-legend { gap: 8px !important; padding: 8px 10px !important; }
          .sitemap-legend span { font-size: 10px !important; }
        }
        @media (min-width: 769px) {
          .sidebar-desktop { display: block !important; }
          .sidebar-mobile-overlay { display: none !important; }
          .header-title-full { display: block !important; }
          .header-title-short { display: none !important; }
          .header-tabs .tab-label-full { display: inline; }
          .header-tabs .tab-label-short { display: none; }
          .hamburger-btn { display: none !important; }
        }
      `}</style>
      <div style={{ background: "#fff", borderBottom: "1px solid #e2e8f0", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, flexShrink: 0 }}>
          <div className="hamburger-btn" onClick={() => setSidebarOpen(!sidebarOpen)} style={{
            display: "none", alignItems: "center", justifyContent: "center", width: 36, height: 36, borderRadius: 8,
            background: sidebarOpen ? "#6366f1" : "#f1f5f9", cursor: "pointer", flexShrink: 0, transition: "all 0.2s",
          }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#94a3b8" }}>Wireframe v2.0</div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8, flexShrink: 0 }}>
          <div className="header-tabs" style={{ display: "flex", background: "#f1f5f9", borderRadius: 8, padding: 3, flexShrink: 0 }}>
            {[
              { id: "dashboard", full: "역할별 대시보드", short: "대시보드" },
              { id: "sitemap", full: "전체 페이지 흐름도", short: "흐름도" },
              { id: "screens", full: "주요 화면", short: "화면" },
            ].map(t => (
              <div className="tab-btn" key={t.id} onClick={() => { setTab(t.id); setDetailPage(null); setSidebarOpen(false); }} style={{
          ...
          <div className="user-info" style={{ display: "flex", alignItems: "center", gap: 8, paddingLeft: 8, borderLeft: "1px solid #e2e8f0" }}>
            {currentUser?.sub_role && (
              <img
                src={getRoleImageByLabel(currentUser.sub_role)}
                alt={currentUser.sub_role}
                title={currentUser.sub_role}
                className="user-avatar"
                style={{
                  width: 36, height: 36, borderRadius: 8, objectFit: "cover", objectPosition: "top",
                  border: "1.5px solid #e2e8f0", flexShrink: 0,
                }}
              />
            )}
            <div style={{ textAlign: "right" }}>
              <div className="user-name" style={{ fontSize: 12, fontWeight: 700, color: "#1e293b" }}>{currentUser?.name || ""}</div>
              <div className="user-sub" style={{ fontSize: 10, color: "#94a3b8" }}>
                {currentUser?.sub_role || ""}
                {currentUser?.sub_role && getParentRoleLabel(currentUser.sub_role) && getParentRoleLabel(currentUser.sub_role) !== currentUser.sub_role && (
                  <span style={{ marginLeft: 4, color: "#cbd5e1" }}>· {getParentRoleLabel(currentUser.sub_role)}</span>
                )}
              </div>
            </div>
            ...
            <button onClick={handleLogout} title="로그아웃" style={{
              ...
              cursor: "pointer", fontSize: 14, color: "#475569", display: "flex", alignItems: "center", justifyContent: "center",
            }}>
              ⏏
            </button>
          </div>
      ...
      {showInstallBanner && (
        <div style={{ background: "linear-gradient(135deg, #6366f1, #8b5cf6)", padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 }}>
          ...
          <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
            <div onClick={handleInstall} style={{ padding: "7px 16px", background: "#fff", borderRadius: 8, fontSize: 12, fontWeight: 700, color: "#6366f1", cursor: "pointer" }}>설치</div>
            <div onClick={() => setShowInstallBanner(false)} style={{ padding: "7px 10px", borderRadius: 8, fontSize: 12, color: "rgba(255,255,255,0.7)", cursor: "pointer" }}>✕</div>
          </div>
        </div>
      )}
          ...
          <>
            ...
            <div style={{
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16, paddingLeft: 8 }}>
                ...
                <div onClick={() => setSidebarOpen(false)} style={{ width: 28, height: 28, borderRadius: 6, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", fontSize: 14 }}>✕</div>
              </div>
              <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", letterSpacing: 1, marginBottom: 12, paddingLeft: 8 }}>역할 선택</div>
              {ROLES.map(role => (
                <div key={role.id} onClick={() => { setSelectedRole(role.id); if (tab !== "screens") setSidebarOpen(false); }} style={{
                  ...
                  {sitemapData.map((group, gi) => (
                    <div key={gi} style={{ marginBottom: 8 }}>
                      ...
                      {group.pages.map((page, pi) => {
                        ...
                        return (
                          <div key={pi} onClick={() => { if (accessible) { setDetailPage(page.name); setSidebarOpen(false); }}} style={{
      ...
      <div style={{ display: "flex", minHeight: "calc(100vh - 65px)" }}>
        {/* Desktop sidebar */}
        <div className="sidebar-desktop" style={{ width: 200, background: "#fff", borderRight: "1px solid #e2e8f0", padding: "20px 12px", flexShrink: 0, overflowY: "auto" }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: "#94a3b8", letterSpacing: 1, marginBottom: 12, paddingLeft: 8 }}>역할 선택</div>
          {ROLES.map(role => (
        ...
        <div className="main-content" style={{ flex: 1, padding: 24, overflow: "auto" }}>
          {tab === "dashboard" && (
