...
## 2026-07-30 운영 배포 및 가역 승인 검증

- 운영 주소: `https://smartms.kr/education-history`
- 서버 페이지네이션 검수 RPC와 API를 운영 Supabase에 적용했다.
- 운영 빌드 및 배포를 완료했고, 비로그인 사용자의 검수 API 접근은 `401`, 일반 성도는 `403`, 관리 권한 사용자는 `200`으로 확인했다.
- 추천 매칭 중 날짜·상태·과정·성도 후보가 명확한 표본 10건을 운영 API로 승인했다.
- 생성된 정식 이력 10건이 공개용 안전 view에 노출되는 것을 확인했다.
- 같은 표본 10건을 즉시 승인 취소하여 모두 soft delete 처리했다.
- 검증 종료 후 활성 `member_education_history`는 0건이며, import row의 `created_history_id`도 모두 해제되었다.
- 승인과 승인 취소 감사 로그가 남아 있으므로 가역 검증 과정은 추적 가능하다.
- 이 검증은 기능 점검이며 실제 자료를 영구 승인한 것이 아니다. 동명이인과 후보 근거는 관리자가 화면에서 확인한 뒤 승인한다.

## 2026-08-05 현황 재확인

위 2026-07-30 기록의 "활성 이력 0건"은 그 시점 기준이다. 이후 상태는 다음과 같다.

- 2026-08-02에 관리자가 화면에서 LMTC 1기 2건(이명진, 이문희)을 직접 검수해 승인했다. 실제 검수의 시작이며 되돌리지 않는다.
- 2026-08-05 기준 활성 `member_education_history`는 2건, `education_import_rows`의 `approved`도 2건이다.
- staging 원본 3,536건 중 recommended 2,228 / ambiguous 125 / unmatched 1,181 / approved 2이며, batch 2건은 모두 `staged` 상태다.
- 과정 미분류 146건은 원본 과정명 10종에서 나온다. `어성경이읽어지네(구약)` 47건과 `어성경이읽어지네(신약)` 36건은 표준 과정 `어성경이읽어지네 구약`·`어성경이읽어지네 신약`과 괄호 표기만 다르므로 별칭 등록으로 해소된다. 나머지 63건(`제 N기 제자반` 51, `평신도지도자반` 10, `7영리` 2)은 대응하는 표준 과정이 없어 신설 여부와 분류 결정이 필요하다.
- unmatched 1,181건은 584명에서 나오며, 이 584명은 현재 활성 성도 명부에 동명이 한 명도 없다. 매칭 누락이 아니라 현재 미등록 인물이다.
- 동명이인 125건은 후보 점수가 전원 90점(이름 완전일치) 동점이다. 후보 중 한 명만 생년월일·전화를 가진 경우 36건, 후보 간 성별 또는 직분이 다른 경우 73건, 후보 전원 정보가 없어 명부만으로는 판별 불가한 경우 16건이다.

### 2026-07-30 검증 흔적의 한글 깨짐 정정

2026-07-30 가역 검증에서 승인 취소를 실행한 경로의 인코딩 문제로, `education_import_rows.review_note` 8건과 soft delete된 `member_education_history.deletion_reason` 10건이 `?? ?? ?? ?? ?? 2026-07-30`으로 저장돼 있었다. 2026-08-05에 해당 값만 골라 `2026-07-30 가역 승인 검증 취소 (원문 인코딩 손상으로 재기록)`로 교체했다. 원문 문구는 복구할 수 없다.

웹 화면과 API는 JSON(UTF-8)으로 사유를 전달하므로 같은 증상이 재현되지 않는다. 저장소 스크립트에도 사유 문자열을 명령행으로 넘기는 경로는 없다. 콘솔에서 직접 RPC를 호출할 때만 셸 인코딩에 주의한다. `education_history_audit_logs`의 과거 기록은 감사 추적이므로 손대지 않았고 깨진 원문이 그대로 남아 있다.
