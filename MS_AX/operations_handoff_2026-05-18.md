...
## AI Directory Review Page - 2026-05-20

Added first-pass AI-assisted review workflow:

- Admin page:
  - `chflow-app/app/admin/ai-review/page.tsx`
  - URL: `/admin/ai-review`
  - Entry point added to `/admin/members`
- API route:
  - `chflow-app/app/api/admin/ai-member-review/run/route.ts`
- Production DB migration applied manually:
  - `chflow-project/supabase/migrations/20260520020000_ai_member_review.sql`

Workflow:

1. Admin searches/selects a current active member.
2. Admin provides a PDF evidence image URL, such as a row crop from the original directory PDF.
3. Server calls OpenAI Responses API with image input and structured JSON output.
4. Result is stored in `ai_member_review_candidates`.
5. Admin page compares DB fields and AI fields.
6. Admin can apply only name, only `sub_role`, both changed fields, hold for review, or ignore.

Safety:

- AI output never directly updates members.
- Updates go through `admin_ai_member_review_decide(...)`.
- Apply is guarded by the original stored `db_name` and `db_sub_role`; if the member changed after the AI candidate was created, the RPC raises and requires reload.
- The page is restricted to admin/office/pastor.

Environment needed before production AI execution:

- `OPENAI_API_KEY`
- Optional:
  - `OPENAI_MEMBER_REVIEW_MODEL`
  - default in code: `gpt-5.4-mini`

Current production verification:

- Table `ai_member_review_candidates` exists.
- Candidate count at creation: 0.
- `npm run build` passed.
- Local route check:
  - `http://localhost:3000/admin/ai-review` returned 200.

Next work:

1. Create/upload original PDF row crop evidence images in a stable location.
2. Batch-create AI candidates from those evidence images.
3. Review candidates in `/admin/ai-review`.
4. Only apply candidates with direct visual evidence and human approval.
