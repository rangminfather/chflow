-- Temporarily route every administrator hotline inquiry and notification to clyawy only.

create or replace function public.is_admin_hotline_operator()
returns boolean
language sql
stable
security definer
...
as $$
  select exists (
    ...
    where p.id = auth.uid()
      and p.status = 'active'
      and lower(coalesce(p.username, '')) = 'clyawy'
  );
$$;

revoke all on function public.is_admin_hotline_operator() from public, anon;
grant execute on function public.is_admin_hotline_operator() to authenticated;
...
begin
  ...
  if public.is_admin_hotline_operator() then
    raise exception '핫라인 담당자는 접수 목록을 이용해 주세요.';
  end if;
  ...
  if v_guide_sender_id is null then
    raise exception '현재 핫라인에 연결할 수 있는 담당자가 없습니다.';
  end if;
...
begin
  ...
  if auth.uid() = v_owner_id then
    ...
    insert into public.messenger_participants (conversation_id, user_id, role, last_read_at)
    values (p_conversation_id, v_operator_id, 'member', v_before_message_at)
    on conflict (conversation_id, user_id) do nothing;
  elsif not public.is_admin_hotline_operator()
    or not public.is_messenger_participant(p_conversation_id, auth.uid())
  then
...
-- Remove every previous hotline recipient except the requester and clyawy.
