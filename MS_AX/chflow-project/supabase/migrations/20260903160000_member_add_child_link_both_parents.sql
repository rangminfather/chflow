-- NOTE (added 2026-09-14 during migration-history reconciliation): this version
-- was applied directly to production on 2026-09-03 (outside the tracked
-- migration flow), which is why it predates 20260909104000_member_add_child_link_both_parents.sql
-- with byte-identical content. This file exists only to make the local
-- migrations folder match production history — do not "fix" the duplication
-- by deleting either file; both are already applied and re-running either is
-- harmless (CREATE OR REPLACE + ON CONFLICT DO UPDATE).
