...
-- 관리자 성도카드는 로그인 아이디를 포함하므로 UI 경로뿐 아니라 RPC 자체를 staff로 제한한다.
-- admin / office / pastor는 기존처럼 조회할 수 있고, 일반 로그인 성도는 실행 시 차단된다.
  ...
  RETURN (
    SELECT jsonb_build_object(
      ...
      'household_members', (
        ...
        WHERE mm.household_id = m.household_id AND mm.id <> m.id
      ),
      'relations', (
        SELECT jsonb_agg(jsonb_build_object(
...
GRANT EXECUTE ON FUNCTION public.admin_member_profile(uuid) TO authenticated, service_role;
