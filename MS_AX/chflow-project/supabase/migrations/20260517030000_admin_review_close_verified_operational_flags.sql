-- Verified MDB-only members without a household are an operational pasture
-- assignment backlog, not a remaining data-review item.
-- Also allow a verified free-text spouse homonym note to suppress spouse mismatch.
    ...
    case
      when nullif(m.spouse_name, '') is not null
       and not (
         ...
         and coalesce(m.review_note, '') like '%배우자 동명이인%'
       )
       and not exists (
         select 1
